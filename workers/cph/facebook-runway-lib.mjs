...
export function buildFacebookCaption(item, hashtags = '') {
  const savedCaption = String(item?.social_caption || '').trim();
  if (savedCaption) return savedCaption;
  ...
  return [
    ...
    excerpt || null,
    permalink ? '' : null,
    permalink ? `Read it here: ${permalink}` : null,
    hashtags ? '' : null,
    hashtags || null,
